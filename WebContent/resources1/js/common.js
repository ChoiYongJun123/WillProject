...
function checkPasswordLayerShow(pwsId,pwsReId,loginId,layerId) {
	...
	if((pws.value.length < 10)||(pws.value.length > 20)){
		var errHtml = "띄어쓰기 없이 영문(대소문자 구별)과 숫자를 조합하여 10~20자 이내로 입력해주세요. <br/>- 특수문자 및 한글 불가 / 4자 이상 중복된 값은 사용 불가합니다.";
		var errStr = "띄어쓰기 없이 영문(대소문자 구별)과 숫자를 조합하여 10~20자 이내로 입력해주세요. 특수문자 및 한글 불가 / 4자 이상 중복된 값은 사용 불가합니다.";
		if(layerId == ''){
			alert(errStr);
		}else{
	...
	for(var i = 0; i < pws.value.length; i++) {
		if(availableCharacter.indexOf(pws.value.charAt(i)) != -1) d++;
		if(availableCharacterStr.indexOf(pws.value.charAt(i)) != -1) a++;
		if(availableCharacterNum.indexOf(pws.value.charAt(i)) != -1) b++;
	}
	...
	for(var i=0; i<availableCharacter.length-4; i++){
	  for( var j=0; j<pws.value.length -4; j++){
			...
			|| (availableCharacter.charAt(i+3) == pws.value.charAt(j)
				...
				var errStr = "특수문자 및 한글 불가 / 4자 이상 연속된 값은 사용 불가합니다.";
				if(layerId == ''){
					alert(errStr);
				}else{
					$("#"+layerId).text(errStr);
...
function viewBranch(branchId) {
	...
	*/
	$.ajax({ 
		url : '/rent/custcnte/branch/viewBranchMap.json',
		type : 'POST',
		dataType : 'json',
...
function getCompanyPopList(revDate, revSeq, revCd){
	...
	if(revCd == "SP"){
		...
		$("#modal-pop-h3-title").text("일급서해자동차정비(주) 외 150여 업체");
		$("#modal-pop-table-title").text("업체명");
		
	}else if(revCd == "SC"){
		$("#modal-pop-h3-title").text("잡플러스 외 10여 업체");
		$("#modal-pop-table-title").text("업체명");
		
	}else if(revCd == "SS"){
		$("#modal-pop-h3-title").text("(주)한솔네트워크서비스 외 120여 업체");
		$("#modal-pop-table-title").text("업체명");
